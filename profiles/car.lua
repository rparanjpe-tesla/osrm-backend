...
function turn_function (angle)
  -- compute turn penalty as angle^2
  return 60*angle*angle/(90.0*90.0)
end
...
--  else
  ...
  -- set the tmc code
  result.tmc_code = way:get_value_by_key("tmc_code")
  ...
  if maxspeed_forward and maxspeed_forward ~= "zero" and maxspeed_forward > 0 then
  ...
  if maxspeed_backward and maxspeed_backward ~= "zero" and maxspeed_backward > 0 then
    result.backward_speed = maxspeed_backward
  end

  if maxspeed_forward and "zero" == maxspeed_forward then
    result.forward_speed = 0
  end
  if maxspeed_backward and "zero" == maxspeed_backward then
